...
class DFRFTLanguageClassifier(nn.Module):  # 重命名类
    """端到端语种分类模型"""

    def __init__(self, frame_length=50, hidden_neurons=32):
        super().__init__()
        self.frame_length = frame_length
        self.P = nn.Parameter(torch.tensor(1.0), requires_grad=True)
        self.Q = nn.Parameter(torch.tensor(1.0), requires_grad=True)
    ...
    def forward(self, x):
        P = torch.clamp(self.P, 0.0, 2.0)
        Q = torch.clamp(self.Q, 0.0, 2.0)
...
def main():
    ...
    history_dfrft = train_model(
        ...
        epochs=100, lr=0.001, model_name="language_classifier"
    )
    # 绘制训练曲线
    ...
    print(f"DFRFT情绪分类器测试准确率: {test_acc_dfrft:.4f}")

    # 加载最佳模型再评估
    print("\n重新加载最佳模型权重并在测试集评估...")
    best_model = DFRFTLanguageClassifier(frame_length=50, hidden_neurons=64)
    best_model.load_state_dict(torch.load("best_language_classifier.pth"))
    best_model.eval()
    ...
    print(f"最佳 DFRFT 情绪分类器测试准确率: {test_acc_best:.4f}")
    ...
    plt.xlabel('分类器类型')
    plt.ylabel('测试准确率')
    plt.title('DFRFT语种分类器性能')  # 修改标题
    plt.ylim(0.0, 1.0)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.text(0, test_acc_dfrft + 0.01, f"{test_acc_dfrft:.4f}", ha='center')
    plt.tight_layout()
    plt.savefig('language_classifier_performance.png')  # 修改文件名
    plt.show()
