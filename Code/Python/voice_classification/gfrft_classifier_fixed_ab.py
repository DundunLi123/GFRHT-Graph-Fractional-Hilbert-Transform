...
def main():
    ...
    history_gfrft = train_model(
        ...
        epochs=50, lr=0.001, model_name="gfrft_language_classifier_fixed_ab"
    )
    # 绘制训练曲线
    plot_training_curves(history_gfrft, "GFRFT_fixed语种分类器")
    # 在测试集上评估
    ...
    # 加载最佳模型再评估
    print("\n重新加载最佳模型权重并在测试集评估...")
    best_model = GFRFTLanguageClassifier(frame_length=50, hidden_neurons=1, adjacency_matrix=A)
    best_model.load_state_dict(torch.load("best_gfrft_language_classifier_fixed_ab.pth"))
    best_model.eval()
    ...
    print(f"最佳 GFRFT_fixed 模型测试准确率: {test_acc_best:.4f}")
    ...
    plt.tight_layout()
    plt.savefig('gfrft_classifier_performance_fixed_ab.png')
    plt.show()
