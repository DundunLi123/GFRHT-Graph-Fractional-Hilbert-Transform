...
import joblib
from dfrft_classifier import DFRFTLanguageClassifier, AMFMExtractor, load_language_data, frame_signal, set_seed,create_frame_dataset
from gfrft_classifier import GFRFTLanguageClassifier, optimize_A,set_seed
...
def main():
    ...
    # 加载DFRFT模型
    model_dfrft = DFRFTLanguageClassifier(frame_length=50, hidden_neurons=64)
    if os.path.exists("best_language_classifier.pth"):
        model_dfrft.load_state_dict(torch.load("best_language_classifier.pth"))
        print("DFRFT模型加载成功")
    else:
        print("警告: 未找到DFRFT模型，请先运行dfrft_classifier.py")
        return
    ...
    if os.path.exists("best_gfrft_language_classifier.pth"):
        model_gfrft.load_state_dict(torch.load("best_gfrft_language_classifier.pth"))
        print("GFRFT模型加载成功")
    else:
        print("警告: 未找到GFRFT模型，请先运行gfrft_classifier.py")
        return
    ...
    history_combined = train_model(
        ...
        epochs=50, lr=0.005, model_name="combined_classifier"
    )
    ...
    best_model.load_state_dict(torch.load("best_combined_classifier.pth"))
    best_model.eval()
    ...
    # 结果比较
    results = {
        'DFRFT': test_acc_dfrft,
        'GFRFT': test_acc_gfrft,
        'Combined': test_acc_best_combined
    }
